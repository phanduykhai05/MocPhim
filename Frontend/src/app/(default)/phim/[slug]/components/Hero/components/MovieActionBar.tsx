...
export const MovieActionBar = ({ movie }: MovieActionBarProps) => {
  ...
  const trailerHref = movie.trailer_url || '#';
  const trailerTarget = movie.trailer_url ? '_blank' : undefined;
    ...
    <div className="relative p-6 lg:p-8 flex flex-col lg:flex-row items-center gap-6 justify-between border-b border-white/5">
      ...
      {isComing ? (
        ...
        >
          <TrailerVideoIcon />
          <span className="whitespace-nowrap text-[16px] leading-none text-[#111827]">Xem Trailer</span>
          <div className="absolute left-0 right-0 bottom-0 flex items-center justify-center text-center text-[11px] leading-6 bg-[#f1f1f1] text-black">Trailer chính thức</div>
        </a>
      ) : hasEpisodes ? (
        <Link
