...
// ─── Main ──────────────────────────────────────────────────────────────────────
export default function CategoryMoviesClient({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    fetchCategoryMovies(slug, { page: pageParam, sort_field: sortField, sort_type: sortType }).then((res) => {
      ...
      // Lấy tên thể loại từ item đầu tiên nếu có
      const firstItem = res?.items?.[0] as unknown as { category?: { slug: string; name: string }[] } | undefined;
      const categories = firstItem?.category ?? [];
  ...
  useEffect(() => {
    ...
    router.replace(`/the-loai/${slug}?${params.toString()}`);
